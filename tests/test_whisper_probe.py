...
from care_asr.probes.whisper_scores_probe import load_audio, run_probe, inspect_scores
...
def test_1_probe_returns_generate_output_with_scores(probe_outputs):
    ...
    assert isinstance(outputs, (GenerateEncoderDecoderOutput, GenerateDecoderOnlyOutput)), (
...
def test_2_scores_list_is_non_empty(probe_outputs):
    ...
    """
    outputs, _ = probe_outputs
    assert isinstance(outputs.scores, (tuple, list)), "outputs.scores should be a tuple or list."
    assert len(outputs.scores) > 0, f"outputs.scores is empty (len={len(outputs.scores)})."
...
def test_3_every_score_tensor_has_vocabulary_dimension(probe_outputs):
    ...
    for step_idx, score_tensor in enumerate(outputs.scores):
        ...
        assert score_tensor.ndim == 2, f"Score tensor at step {step_idx} expected rank 2 (batch_size, vocab_size), got shape {score_tensor.shape}."
        assert score_tensor.shape[-1] == expected_vocab_size or score_tensor.shape[-1] > 50000, (
...
def test_4_generated_transcription_is_non_empty(probe_outputs):
    ...
    assert len(transcription.strip()) > 0, "Generated transcription is empty or whitespace only."
