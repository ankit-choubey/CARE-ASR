...
from care_asr.contracts.retrieval_input import (
    RetrievalCandidatesInput, EntityQuery, SemanticCandidate, PhoneticCandidate
)
...
def base_retrieval_input():
    return RetrievalCandidatesInput(
...
def test_single_candidate(evaluator, base_retrieval_input):
    query = EntityQuery(
        ...
        end_char=7,
        semantic_candidates=[
            SemanticCandidate(concept_id="C0004057", canonical_name="Aspirin", similarity_score=0.95, cui="C0004057")
        ],
        phonetic_candidates=[
            PhoneticCandidate(concept_id="C0004057", canonical_name="Aspirin", phonetic_distance=1.0, cui="C0004057")
        ]
    )
    base_retrieval_input.entity_queries.append(query)
    
    aligned_entities = [{
        "entity": {"entity_id": "span-1", "entity_text": "Aspirin", "category": "MED", "start_char": 0, "end_char": 7},
...
def test_multiple_candidates_and_tie_scores(evaluator, base_retrieval_input):
    query = EntityQuery(
        ...
        semantic_candidates=[
            ...
            SemanticCandidate(concept_id="C2", canonical_name="Paracetamol", similarity_score=0.85, cui="C2") # Tie semantic
        ],
        phonetic_candidates=[
            PhoneticCandidate(concept_id="C1", canonical_name="Acetaminophen", phonetic_distance=1.0, cui="C1") # C1 wins tie due to phonetic
        ]
    )
    base_retrieval_input.entity_queries.append(query)
    
    aligned_entities = [{
        "entity": {"entity_id": "span-2", "entity_text": "Tylenol", "category": "MED", "start_char": 10, "end_char": 17},
...
def test_threshold_rejection(evaluator, base_retrieval_input):
    ...
    aligned_entities = [{
        "entity": {"entity_id": "span-3", "entity_text": "BadDrug", "category": "MED", "start_char": 0, "end_char": 7},
        "average_asr_confidence": 0.90,
        "maximum_entropy": 0.10
    }]
...
def test_empty_retrieval(evaluator, base_retrieval_input):
    ...
    aligned_entities = [{
        "entity": {"entity_id": "span-4", "entity_text": "Nothing", "category": "MED", "start_char": 0, "end_char": 7},
        "average_asr_confidence": 0.90,
        "maximum_entropy": 0.10
    }]
...
def test_unknown_category(evaluator, base_retrieval_input):
    aligned_entities = [{
        "entity": {"entity_id": "span-5", "entity_text": "Unknown", "category": "INVALID", "start_char": 0, "end_char": 7},
        "average_asr_confidence": 0.90,
        "maximum_entropy": 0.10
    }]
