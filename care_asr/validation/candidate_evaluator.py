...
from care_asr.contracts.retrieval_input import RetrievalCandidatesInput, EntityQuery
...
class CandidateEvaluator:
    ...
    def _load_weights(self) -> dict[str, float]:
        ...
        weights = yaml_config.get("weights", {
    ...
    def evaluate_candidates(
        ...
        for aligned_ent in aligned_entities:
            ...
            if asr_conf < threshold_rules.get("min_asr_confidence", 0.0) or entropy > threshold_rules.get("max_entropy", 1.0):
            ...
            if validated_cands:
                ...
                logger.debug(f"Entity '{entity_data.get('entity_text')}' top candidate: '{top_cand.canonical_name}' (Score: {top_cand.composite_utility_score:.2f})")
    ...
    def _validate_candidate(self, category: str, sem_sim: float, phon_dist: float, asr_conf: float, entropy: float) -> bool:
    ...
    def _rank_candidates(self, evaluated_cands: list[dict[str, Any]]) -> list[ValidatedCandidate]:
        ...
        for rank, cand in enumerate(evaluated_cands, start=1):
            ranked_list.append(ValidatedCandidate(
                candidate_rank=rank,
    ...
    def _validate_and_rank(
        self, merged_candidates: dict[str, dict[str, Any]], category: str, asr_conf: float, entropy: float
